...
public class StatementsProving {
    ...
    private void cutConditions() throws MathLogicException {
        for (int i = 0; i < varByIndex.size(); i++) {
            cutConditionIndex = i;
            String alpha = varByIndex.get(cutConditionIndex);
            globalProof.addAll(LogConnects.loadProof("Aor!A", alpha));
